...
class WhisperLargeV3Transcriber:
    ...
    def __init__(self, model_directory: Path, device: str = "auto"):
        ...
        self.model = AutoModelForSpeechSeq2Seq.from_pretrained(
            model_directory, torch_dtype=dtype, low_cpu_mem_usage=True, use_safetensors=True
        ).to(self.device).eval()
        self.pipeline = pipeline(
            ...
            torch_dtype=dtype, device=0 if self.device.startswith("cuda") else -1,
        )

    @torch.inference_mode()
    def transcribe(self, audio_path: Path) -> dict:
        return self.pipeline(
            str(audio_path), chunk_length_s=30, stride_length_s=(5, 5), return_timestamps=True,
...
def ensure_whisper_large_v3(model_directory: Path, download: bool) -> Path:
    ...
    if not download:
        raise FileNotFoundError(
            f"Whisper large-v3 is missing: {model_directory}. Run again with "
            "--download-asr-model to download openai/whisper-large-v3."
        )
